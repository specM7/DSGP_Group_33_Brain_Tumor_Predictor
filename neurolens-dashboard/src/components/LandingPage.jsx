...
export default function LandingPage({ onNavigateLogin }) {
    ...
    useEffect(() => {
        const sectionIds = ['features', 'how-it-works', 'technology', 'faq'];
    ...
    const navLinks = ['Features', 'How It Works', 'Technology', 'FAQ'];

    const [openFaq, setOpenFaq] = useState(null);
    const faqs = [
        { q: 'What is NeuroLens?', a: 'NeuroLens is an AI-powered medical imaging platform that uses deep learning to detect and classify brain tumors from MRI scans with clinical-grade accuracy in under 3 seconds.' },
        { q: 'How accurate is the AI detection?', a: 'Our CNN model achieves 95% accuracy on brain tumor classification, trained on over 10,000 labeled MRI scans. It can classify tumors as glioma, meningioma, pituitary, or healthy tissue.' },
        { q: 'What image formats are supported?', a: 'NeuroLens supports DICOM, PNG, and JPG formats. We recommend a minimum resolution of 512×512 pixels for accurate analysis. Drag and drop upload is also supported.' },
        { q: 'Is my patient data secure?', a: 'Yes. All data is processed with enterprise-grade encryption. We follow HIPAA-aware practices, and scans are not stored permanently unless you choose to save them.' },
        { q: 'Can I use NeuroLens for research purposes?', a: 'Absolutely. NeuroLens is designed for radiologists, hospitals, and AI researchers. It supports large-scale automated analysis and benchmarking of deep learning architectures for medical imaging.' },
        { q: 'Is NeuroLens free to use?', a: 'NeuroLens is currently available for free during our research and development phase. We aim to keep a free tier available for educational and research use.' },
    ];
    ...
    return (
        <div className={`nl-landing ${isDark ? '' : 'nl-light'}`}>
            ...
            <nav className={`nl-navbar ${navScrolled ? 'scrolled' : ''}`}>
                <div className="nl-navbar-inner">
                    ...
                    <div className="nl-nav-links">
                        {navLinks.map(l => {
                            ...
                            return (
                                <button key={l} className={`nl-nav-link ${isActive ? 'active' : ''}`}
                ...
                {/* Animated glow border on scroll */}
                {navScrolled && <div className="nl-navbar-glow-border" />}
            ...
            {/* ═══════════ FAQ ═══════════ */}
            <section id="faq" className="nl-section">
                <div className="nl-section-container">
                    <div ref={addRef} className="nl-reveal nl-section-header">
                        <span className="nl-section-tag">
                        ...
                        <p className="nl-section-desc">
                            Everything you need to know about NeuroLens and AI-powered brain tumor detection.
                        </p>
                    </div>

                    <div ref={addRef} className="nl-reveal nl-faq-list">
                        {faqs.map((faq, i) => (
                            <div
                                key={i}
                                className={`nl-faq-item ${openFaq === i ? 'open' : ''}`}
                                style={{ '--d': i }}
                            >
                                <button
                                ...
                                >
                                    <span>{faq.q}</span>
                                    <ChevronDown size={18} className="nl-faq-chevron" />
                                </button>
